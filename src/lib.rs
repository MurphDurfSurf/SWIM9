    ...
    >
{
    ...
    // realised I will be using these claulations alot so I just added some simlple methods
    pub fn byte_index(&self, num: usize) -> usize {
        num / 8
    }

    pub fn bit_pos(&self, num: usize) -> usize {
        num % 8 
    }

    pub fn directory_exists(&mut self) -> bool {
        let mut block = [0u8; BLOCK_SIZE];
        self.disk.read(INODE_FULL_BLOCK, &mut block).unwrap();

        block[0] != 0 // returns the bool, if the 0 index is not 0, then we get a True
    ...
    pub fn load_file_bytes(&mut self, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        ...
        for i in 0..self.num_inode_blocks() {
            ...
            self.disk.read(block_num.into(), &mut self.block_buffer).unwrap();

            let bytes_to_copy = 
            if offset + BLOCK_SIZE > MAX_FILE_BYTES {
                MAX_FILE_BYTES - offset // Don't exceed the file content buffer
            } else {
                BLOCK_SIZE // Copy the full block if there's enough space in the buffer
            };
    
            // Copy the block's contents into the file content buffer
            self.file_content_buffer[offset..offset + bytes_to_copy]
                .copy_from_slice(&self.block_buffer[..bytes_to_copy]);
    
            // Update the offset to point to the next available space in the file content buffer
    ...
    pub fn save_file_bytes(&mut self, inode: &Inode<MAX_FILE_BLOCKS, BLOCK_SIZE>) {
        ...
        let mut offset = 0;
        for i in 00..inode.blocks_used() {
            let reference = inode.blocks[i];

            // Read the current block's data into the block buffer
            self.disk.read(reference.into(), &mut self.block_buffer).unwrap();

            // Calculate how many bytes to copy from the file content buffer
            let bytes_to_copy = if offset + BLOCK_SIZE > MAX_FILE_BYTES {
                MAX_FILE_BYTES - offset // Don't exceed the file content buffer
            } else {
                BLOCK_SIZE // Copy the full block if there's enough space in the buffer
            };
            // Copy the appropriate part of the file_content_buffer into the block_buffer
            self.block_buffer[..bytes_to_copy]
                .copy_from_slice(&self.file_content_buffer[offset..offset + bytes_to_copy]);

            // Write the modified block buffer back to the disk
            self.disk.write(reference.into(), &self.block_buffer).unwrap(); 

            // Update the offset to point to the next available space in the file content buffer
            offset += bytes_to_copy;

            // If we've written all the bytes in the file content buffer, stop
            if offset >= MAX_FILE_BYTES {
                break;
            }
         }
    ...
    pub fn activate_bit(&mut self, bit: usize, block: usize) {
        ...
        self.block_buffer[self.byte_index(bit)] |= 1 << self.bit_pos(bit); // |= is | assignment, self.byte_index & bit_pos on ln 150
    ...
    fn request_data_block(&mut self) -> Result::<u8, FileSystemError> {
        ...
        for i in 0..self.num_data_blocks() { // For some reason it dosn't like self.first_data_block() as starting position
            let block = self.first_data_block() + i;
            
            if self.block_buffer[self.byte_index(block)] & (1 << self.bit_pos(block)) == 0 {
    ...
    pub fn inode_table_inode(&mut self) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        let bytes_stored = self.num_inode_entries();

        let mut blocks = [0u8; MAX_FILE_BLOCKS];
        let first_inode_block = 2;
        let last_inode_block = self.first_data_block();

        for i in first_inode_block..last_inode_block {
            blocks[(i - first_inode_block) as usize] = i as u8;
        }

        Inode {
            bytes_stored: bytes_stored as u16,
            blocks,
        }
    ...
    fn load_inode(&mut self, inode_num: usize) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        let p_inode = self.inode_table_inode(); 
        self.load_file_bytes(&p_inode);
        let offset = inode_num * self.num_inode_bytes(); 
        Inode::from_bytes(&self.file_content_buffer[offset..offset + self.num_inode_bytes()])
    }
    ...
    fn mark_inode_blocks_in_use(&mut self) { // NEED TO FIX --------------------------------------------------------------
        ...
        self.activate_bit(1, DATA_FULL_BLOCK);
        //todo!("Call activate_bit() for each inode block to mark all of the inode blocks as being in use.")
    }

    fn initialize_new_file( // FAILING FIRST TEST -------------------------------------------------------------------------
        &mut self,
        inode_num: usize,
        first_block: u8,
    ) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
            // Step 1: Create a new inode using first_block.
        let mut inode = Inode {bytes_stored: 0, blocks: [0; MAX_FILE_BLOCKS],};
        ...
        // Step 5: Return the newly created inode.
        return inode
    }
    ...
    fn directory_inode(&mut self) -> Inode<MAX_FILE_BLOCKS, BLOCK_SIZE> {
        ...
        if self.directory_exists() { 
            ...
            let block = self.request_data_block().unwrap(); 
            self.initialize_new_file(0, block)
        }
    ...
    ) -> Result::<(usize, [[u8; MAX_FILENAME_BYTES]; MAX_FILES_STORED]), FileSystemError>
    {
        ...
        .ok_or(FileSystemError::DirectoryEmpty)?;
        if first_zero <= 1 { 
            return Err(FileSystemError::DirectoryEmpty); 
        } 
        self.load_directory()?; 
        let mut count = 0; 
        for i in 1..first_zero { 
            let offset = i * MAX_FILENAME_BYTES; 
            filenames[count].copy_from_slice(&self.file_content_buffer[offset..offset + MAX_FILENAME_BYTES]);
            count += 1;
        }
        Ok((count, filenames))
    ...
    pub fn inode_for(
        ...
        for i in 0..MAX_FILES_STORED { 
            let offset = i * MAX_FILENAME_BYTES; 
            let file_entry = &self.file_content_buffer[offset..offset + MAX_FILENAME_BYTES]; 
            let null_terminated = file_entry.iter().position(|&b| b == 0).unwrap_or(MAX_FILENAME_BYTES); 
            if &file_entry[..null_terminated] == filename.as_bytes() { 
                return Ok((i, self.load_inode(i)));
            }
        }
        ...
        //     Return the index along with the inode (using `Inode::from_bytes()`)
    ...
    pub fn open_read(&mut self, filename: &str) -> Result::<usize, FileSystemError> {
        ...
        if self.open_inodes[inode_num] {
            return Err(FileSystemError::AlreadyOpen);  // File is already open
        }
        // Find the lowest available file descriptor
        let fd = self.find_lowest_fd().ok_or(FileSystemError::TooManyOpen(MAX_OPEN))?;

        // Create a FileInfo for the read operation
        let mut info = FileInfo::read(inode, inode_num);

        // Read the first block of the file into the block buffer
        let block_num = info.inode.blocks[0] as usize;
        self.disk.read(block_num, &mut info.block_buffer).unwrap();

        // Update the open file table and mark the inode as open
        self.open[fd] = Some(info);
    ...
    pub fn open_create(&mut self, filename: &str) -> Result::<usize, FileSystemError> {
        ...
        // Try to find the inode for the given filename
        match self.inode_for(filename) {
            Ok((inode_num, inode)) => {
                // If the file exists, reset it
                self.open_create_reset(inode_num, inode)
            }
            Err(e) => match e {
    ...
    pub fn open_create_new(&mut self, filename: &str) -> Result::<usize, FileSystemError> {
        ...
        // Create a new FileInfo entry for the file
        let info = FileInfo::write(inode, inode_num);
        self.open[fd] = Some(info);
        self.open_inodes[inode_num] = true;
    ...
    fn create_directory_entry(
        ...
        // Copy the filename into the content buffer at the correct position
        for i in 0..MAX_FILENAME_BYTES {
            self.file_content_buffer[offset + i] = if i < bytes.len() { bytes[i] } else { 0 };
        }
    
        // Handle the case when more space is needed in the directory block
        directory_inode.bytes_stored = (offset + MAX_FILENAME_BYTES) as u16;
        if  directory_inode.bytes_stored > BLOCK_SIZE as u16 {
            // Request a new data block if the directory block is full
    ...
    pub fn close(&mut self, fd: usize) -> Result::<(), FileSystemError> {
        ...
        let inode_num = data.inode_num;
        let writing = data.writing;
        let inode = data.inode.clone();
        
        if writing {
            self.save_inode(inode_num, &inode);  // Save the inode if the file was being written to
        }
        
        self.open[fd] = None;  // Mark the file descriptor as unused
        self.open_inodes[inode_num] = false;  // Mark the inode as unused
        Ok(())
    }
    ...
    pub fn read(&mut self, fd: usize, buffer: &mut [u8]) -> Result::<usize, FileSystemError> {
        ...
        if info.writing {
            return Err(FileSystemError::NotOpenForRead);  // Cannot read if file is open for writing
        }
    
        let mut count = 0;
        while count < buffer.len() && info.offset < info.inode.bytes_stored as usize {
            if info.offset % BLOCK_SIZE == 0 {
                let block = info.inode.blocks[info.current_block] as usize;
                self.disk.read(block, &mut info.block_buffer).unwrap();  // Read the block from disk
            }
            
            buffer[count] = info.block_buffer[info.offset % BLOCK_SIZE];  // Copy byte from block buffer
            info.offset += 1;
    
            if info.offset % BLOCK_SIZE == 0 {
                info.current_block += 1;  // Move to the next block if current block is filled
            }
            count += 1;
        }
    
        Ok(count)  // Return the number of bytes read
    }
    ...
    pub fn write(&mut self, fd: usize, buffer: &[u8]) -> Result::<(), FileSystemError> {
        for &b in buffer {
            ...
            if !info.writing {
                return Err(FileSystemError::NotOpenForWrite);  // File must be open for writing
            }
            if info.offset >= MAX_FILE_BYTES {
                return Err(FileSystemError::FileTooBig(MAX_FILE_BYTES));  // File exceeds max size
            }
    
            info.block_buffer[info.offset % BLOCK_SIZE] = b;  // Write the byte to the block buffer
            info.offset += 1;
    
            let block_full;
            let current_block;
            {
                let info = self.open[fd].as_ref().unwrap();
                block_full = info.offset % BLOCK_SIZE == 0;
                current_block = info.current_block;
            }
    
            if block_full {
                let (block_num, buffer_copy) = {
                    let info = self.open[fd].as_ref().unwrap();
                    (info.inode.blocks[current_block] as usize, info.block_buffer)
                };
    
                self.disk.write(block_num, &buffer_copy).unwrap();  // Write the block to disk
    
                // Check if we need a new block
                let needs_new_block;
                {
                    let info = self.open[fd].as_ref().unwrap();
                    needs_new_block = info.current_block + 1 < MAX_FILE_BLOCKS;
                }
    
                if needs_new_block {
                    let next_block = self.request_data_block()?;
                    let info = self.open[fd].as_mut().unwrap();
                    info.current_block += 1;
                    info.inode.blocks[info.current_block] = next_block;
                } else {
                    // return error
                    let info = self.open[fd].as_mut().unwrap();
                    info.current_block += 1;
        ...
        // Update the inode with the new bytes stored
        let (block_num, buffer_copy) = {
            let info = self.open[fd].as_ref().unwrap();
            (info.inode.blocks[info.current_block] as usize, info.block_buffer)
        };
        self.disk.write(block_num, &buffer_copy).unwrap();
        let info = self.open[fd].as_mut().unwrap();
        info.inode.bytes_stored = info.offset as u16;
        // save inode
    ...
    pub fn open_create_reset(
        ...
        // Clear the blocks in the DATA_FULL_BLOCK for the file
        for i in 0..inode.blocks_used() {
            let block = inode.blocks[i] as usize;
            let byte = block / 8;
            let bit = block % 8;
            self.block_buffer[byte] &= !(1 << bit);  // Clear the bit in the bitmap
        }
        self.disk.write(DATA_FULL_BLOCK, &self.block_buffer).unwrap();  // Write the updated bitmap
    
        // Reset inode's stored data
        inode.bytes_stored = 0;
        for i in 1..MAX_FILE_BLOCKS {
            inode.blocks[i] = 0;  // Clear the blocks array
        }
        
        self.save_inode(inode_num, &inode);  // Save the updated inode
        
        // Find the lowest available file descriptor
        let fd = self.find_lowest_fd().ok_or(FileSystemError::TooManyOpen(MAX_OPEN))?;
        
        // Create a new FileInfo entry and assign it to open
        let info = FileInfo::write(inode, inode_num);
        self.open[fd] = Some(info);
        self.open_inodes[inode_num] = true;  // Mark the inode as open
        
        Ok(fd)  // Return the file descriptor
    }
    ...
    pub fn open_append(&mut self, filename: &str) -> Result::<usize, FileSystemError> {
        ...
        // Set up the FileInfo for the append operation
        let mut info = FileInfo::write(inode, inode_num);
        info.offset = info.inode.bytes_stored as usize;
        info.current_block = info.offset / BLOCK_SIZE;
        
        let block_num = info.inode.blocks[info.current_block] as usize;
        self.disk.read(block_num, &mut info.block_buffer).unwrap();  // Read the current block
    
        // Assign the FileInfo to the open table
        self.open[fd] = Some(info);
        self.open_inodes[inode_num] = true;  // Mark the inode as open
...
impl<const MAX_BLOCKS: usize, const BLOCK_SIZE: usize> Inode<MAX_BLOCKS, BLOCK_SIZE> {
    ...
    pub fn from_bytes(buffer: &[u8]) -> Self {
        let bytes_stored = u16::from_le_bytes([buffer[0], buffer[1]]);
    ...
    pub fn update_inode_table(&self, offset: usize, table_buffer: &mut [u8]) {
        table_buffer[offset] = self.bytes_stored as u8;

        // Update the high-order bits of bytes_stored
        table_buffer[offset + 1] = (self.bytes_stored >> 8) as u8;
    
        // Copy the blocks into the table buffer
        let start_index = offset + 2;
        for (i, &block) in self.blocks.iter().enumerate() {
            table_buffer[start_index + i] = block;
        }
...
mod tests {
    ...
    fn test_initialize_new_file_1() {
        ...
        assert_eq!(new_file_inode.blocks, [7; 8]);

        sys.disk.read(1, &mut sys.block_buffer).unwrap();
        assert_eq!(sys.block_buffer[0], 128);
        for i in 1..sys.block_buffer.len() {
            assert_eq!(sys.block_buffer[i], 0);
        }
    }
    ...
    fn test_file_not_found() {
        ...
        match sys.open_read("one.tx") {
            Result::Ok(_) => panic!("Shouldn't have found the file"),
    ...
    fn test_file_not_open() {
        ...
        match sys.read(fd + 1, &mut buffer) {
            Result::Ok(_) => panic!("Should be an error!"),
    ...
    fn test_not_open_for_read() {
        ...
        match sys.read(f1, &mut buffer) {
            Result::Ok(_) => panic!("Should not work!"),
    ...
    fn test_filename_too_long() {
        ...
        match sys.open_create("this_is_an_exceedingly_long_filename_to_use.txt") {
            Result::Ok(_) => panic!("This should be an error"),
            Err(e) => assert_eq!(
                e,
